import type { GetUserByCifUseCase } from '../../application/useCases/GetUserByCifUseCase.ts';
...
export class GetUserIdByCifrController {
  ...
  execute = async (
    ...
    try {
      const userIdByCif = await this.getUserByCifUseCase.execute(
        req.body.CIF as string
      );
      if (userIdByCif !== null) {
        const error: CustomError = throwError(
          'El CIF indicado ya esta en uso',
          403
        );
        throw error;
      }
